...
var Load_userlist = function(userlist,pageIndex,pageSize) {
	...
	if (userlist[1][0] != 0) {
		...
		for (var i = 0; i <pageSize; i++) {
			adduser_tr(userlist[0][i].menu_id, userlist[0][i].menu_name, userlist[0][i]
				.premission);
		}
	}
}
...
//用户列表详情
function adduser_tr(menu_id, menu_name, premissions) {
	console.log("premissions长度"+premissions.lenth);
	var oTr = document.createElement('tr');
	var oTd1 = document.createElement('td');
	var oInput = document.createElement('input');
	oInput.setAttribute('type', 'checkbox');
	oInput.setAttribute('name', 'item');
	oInput.setAttribute('id', menu_id);
	//创建选择按钮
	oTd1.appendChild(oInput);
	//创建单元格

	var oTd2 = document.createElement('td');
	oTd2.innerHTML = menu_name;

	var oTd3 = document.createElement('td');
	oTd3.setAttribute('name', 'menu_name_item');
	oTd3.className = 'menu_name_item';
	var elementInput1 = document.createElement('input');
	elementInput1.setAttribute('type', 'checkbox');
	elementInput1.setAttribute('name', 'element_item1');
	elementInput1.setAttribute('id', 'element_item1');
	if(premissions.length!=0&&premissions[0].element_id==1){
		console.log(111111111)
		elementInput1.setAttribute('checked', 'true');
	}
	oTd3.appendChild(elementInput1);
	
	var oTd4 = document.createElement('td');	
	oTd4.setAttribute('name', 'menu_name_item');
	oTd4.className = 'menu_name_item';
	var elementInput2 = document.createElement('input');
	elementInput2.setAttribute('type', 'checkbox');
	elementInput2.setAttribute('name', 'element_item2');
	elementInput2.setAttribute('id', 'element_item2');
	if(premissions.length!=0&&premissions[1].element_id==2){
		elementInput2.setAttribute('checked', 'true');
	}
	oTd4.appendChild(elementInput2);


	var oTd5 = document.createElement('td');
	var elementInput3 = document.createElement('input');
	elementInput3.setAttribute('type', 'checkbox');
	elementInput3.setAttribute('name', 'element_item3');
	elementInput3.setAttribute('id', 'element_item3');
	if(premissions.length!=0&&premissions[2].element_id==3){
		elementInput3.setAttribute('checked', 'true');
	}
	oTd5.appendChild(elementInput3);

	var oTd6 = document.createElement('td');
	var elementInput4 = document.createElement('input');
	elementInput4.setAttribute('type', 'checkbox');
	elementInput4.setAttribute('name', 'element_item4');
	elementInput4.setAttribute('id', 'element_item4');
	if(premissions.length!=0&&premissions[3].element_id==4){
		elementInput4.setAttribute('checked', 'true');
	}
	oTd6.appendChild(elementInput4);
	...
	Pagination.prototype.render = function() {
			...
			if (i >= between.min && i <= between.max) {
				...
				li.addEventListener('click', function() {
					if (this.dataset.index != _this.options.pageIndex) {
						...
						Ajax_Pagination_query("http://localhost:8080/pmsystem/menuinfo",this.dataset.index,pageSize_a,"query");
		...
		if (numberPrePageArray.length != 0) {
			...
			this.select.addEventListener('change', function() {
				...
				Ajax_Pagination_query("http://localhost:8080/pmsystem/menuinfo",
					1,value_s,"query");
