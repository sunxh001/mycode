...
var Load_userlist = function(userlist,pageIndex,pageSize) {
	...
	if (userlist[1][0] != 0) {
		...
		for (var i = 0; i < pageSize; i++) {
			adduser_tr(i+1,userlist[0][i].user_id, userlist[0][i].user_name, userlist[0][i]
				.dapartment_name, userlist[0][i].password);
		}
	}
}
//分页实例
...
//用户列表详情
function adduser_tr(id, user_id, user_name, department_name, password) {
	var oTr = document.createElement('tr');
	var oTd1 = document.createElement('td');
	var oInput = document.createElement('input');
	oInput.setAttribute('type', 'checkbox');
	oInput.setAttribute('name', 'item');
	oInput.setAttribute('id', user_id);
	//创建选择按钮
	...
	oTd3.setAttribute('name', 'user_id_item');
	oTd3.className = 'user_id_item';
	oTd3.innerHTML = user_id;

	var oTd4 = document.createElement('td');
	oTd4.setAttribute('name', 'user_name_item');
	oTd4.className = 'user_name_item';
	oTd4.innerHTML = user_name;

	var oTd5 = document.createElement('td');
	oTd5.innerHTML = department_name;

	var oTd6 = document.createElement('td');
	oTd6.innerHTML = password;
	...
	Pagination.prototype.render = function() {
			...
			if (i >= between.min && i <= between.max) {
				...
				li.addEventListener('click', function() {
					if (this.dataset.index != _this.options.pageIndex) {
						...
						Ajax_Pagination_query("http://localhost:8080/pmsystem/userinfo",this.dataset.index,pageSize_a,"query");
		...
		if (numberPrePageArray.length != 0) {
			...
			this.select.addEventListener('change', function() {
				...
				Ajax_Pagination_query("http://localhost:8080/pmsystem/userinfo",
					1,value_s,"query");
